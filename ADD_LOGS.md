...
- `session_label` (readable, time-sortable `YYYY-MM-DD HH:MM:SS.mmm INITIALS`; set as BOTH a log attribute (so Loki panels can filter by it) AND a `game_session` span attribute (so Tempo can enumerate it); drives the dashboard's Tempo-backed, newest-first Session picker)
